...
public static class DeviceGuiTests {
 ...
 static void Run(string[] args){string work=Path.GetFullPath(args[0]),expected=args[1],memory=args[2],root=Path.Combine(work,"gui-actual");Directory.CreateDirectory(root);Application.EnableVisualStyles();string ini=Path.Combine(Engine.PspRoot(memory),"SYSTEM","ppsspp.ini"),before=File.ReadAllText(ini);
