...
public sealed class LauncherForm : Form {
 readonly string settingsPath,tools;
 ...
 Button check,install,play,stop; ProgressBar bar; TableLayoutPanel paths,components;
 ...
 public LauncherForm(string root,bool previewMode) {
  preview=previewMode;settingsPath=Path.Combine(root,"launcher-settings.json");tools=Path.Combine(root,"tools");
  ...
  var outer=new TableLayoutPanel{Dock=DockStyle.Fill,Padding=new Padding(26,18,26,18),ColumnCount=1,RowCount=9};Controls.Add(outer);
  foreach(float h in new[]{88F,68F,202F,119F,32F,62F,32F})outer.RowStyles.Add(new RowStyle(SizeType.Absolute,h));
  ...
  var options=new Panel{Dock=DockStyle.Fill,BackColor=panel,Padding=new Padding(12)};outer.Controls.Add(options,0,3);
  ...
  selectionSummary=Label("선택한 자료만 설치합니다. 기존 HD·치트·세이브는 자동 백업합니다.",muted);selectionSummary.Font=new Font("맑은 고딕",9);outer.Controls.Add(selectionSummary,0,4);
  var buttons=new FlowLayoutPanel{Dock=DockStyle.Fill,FlowDirection=FlowDirection.LeftToRight,WrapContents=false,Padding=new Padding(0,9,0,4)};outer.Controls.Add(buttons,0,5);
  ...
  var progressPanel=new TableLayoutPanel{Dock=DockStyle.Fill,ColumnCount=2};progressPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,65));progressPanel.ColumnStyles.Add(new ColumnStyle(SizeType.Percent,35));outer.Controls.Add(progressPanel,0,6);
  ...
  log=new TextBox{Dock=DockStyle.Fill,Multiline=true,ReadOnly=true,ScrollBars=ScrollBars.Vertical,BackColor=panel,ForeColor=muted,BorderStyle=BorderStyle.None,Font=new Font("맑은 고딕",9)};outer.Controls.Add(log,0,7);
  var footer=new Label{Dock=DockStyle.Fill,Text="원본 ISO·에뮬레이터는 별도로 준비하세요. PSP 실기 호환은 미검증입니다.",ForeColor=muted,TextAlign=ContentAlignment.BottomLeft,Font=new Font("맑은 고딕",9)};outer.Controls.Add(footer,0,8);
  ...
  if(!preview)Shown+=(s,e)=>CheckLatest();else{available.Text="새 버전  v8e + HD v39\n업데이트 버튼으로 적용";current.Text="설치됨  본편 v8d · HD v38\n치트 미설치 · 클리어 세이브 미설치";log.Text="보유한 일본판 원본 ISO와 PPSSPP 경로를 한 번 지정해 주세요.\r\nHD 리팩·치트·클리어 세이브는 필요한 항목만 선택할 수 있습니다.";}
 }
 ...
 TextBox PathRow(int row,string caption,string value,bool folder,string filter) {
  ...
  var b=Button("찾기…",()=>{
   ...
   else using(var d=new OpenFileDialog{Title=caption+" 선택",Filter=filter,CheckFileExists=true})if(d.ShowDialog(this)==DialogResult.OK){box.Text=d.FileName;if(row==1&&memstick.Text==""){string guess=Path.Combine(Path.GetDirectoryName(d.FileName),"memstick");if(Directory.Exists(guess))memstick.Text=guess;}}
 ...
 void Busy(bool value){busy=value;check.Enabled=!value;install.Enabled=!value&&release!=null;play.Enabled=!value&&File.Exists(config.GameIso);stop.Enabled=value;paths.Enabled=!value;components.Enabled=!value;if(!value){bar.Style=ProgressBarStyle.Continuous;RefreshInstalled();}}
